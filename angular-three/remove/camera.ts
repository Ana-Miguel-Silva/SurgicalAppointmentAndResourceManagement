...
/**
 * Camera class manages camera settings, including perspective and orthographic cameras.
 * Parameters:
 * - view: string
...
export class Camera {
  ...
  private playerDirection: number = 0.0;
  ...
  private setViewingParameters(): void {
    const orientation = new Orientation(
      this.orientation.h + this.playerDirection,
      this.orientation.v
    );
  ...
  private setProjectionParameters(): void {
    ...
    if (this.aspectRatio < 1.0) {
      fov = 2.0 * THREE.MathUtils.radToDeg(
        Math.atan(
  ...
  private setActiveProjection(projection: string): void {
    this.projection = projection;
    this.object = this.projection !== "orthographic" ? this.perspective : this.orthographic;
  }
  ...
  private initialize(): void {
    ...
    this.setActiveProjection(this.view !== "orthographic" ? "perspective" : "orthographic");
  }
  ...
  setViewport(multipleViews: boolean): void {
    this.viewport = multipleViews
      ? this.multipleViewsViewport.clone()
