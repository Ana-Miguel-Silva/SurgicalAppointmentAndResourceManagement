...
export default class MedicalConditionService implements IMedicalConditionService {
  constructor(@Inject(config.repos.medicalCondtion.name) private MedicalConditionRepo: IMedicalConditionRepo) {}
    ...
    try {
      ...
      if (!MedicalCondition || MedicalCondition.length === 0) {
        ...
        const MedicalConditionDTOResult = MedicalCondition.map(
          medicalRecord => MedicalConditionMap.toDTO(medicalRecord) as IMedicalConditionDTO,
        );
  ...
  public async getMedicalConditionAll(
    ...
    try {
      ...
      if (!MedicalCondition || MedicalCondition.length === 0) {
        ...
        const MedicalConditionTOResult = MedicalCondition.map(
          medicalCondition => MedicalConditionMap.toDTO(medicalCondition) as IMedicalConditionDTO,
        );
